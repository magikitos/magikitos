    ...
    for (const [width, height] of [[1440, 900], [768, 1024], [390, 844]]) {
      ...
      for (const variant of variants) {
        ...
        const result = await page.evaluate(({ variant, width, speeds, strides }) => {
          ...
          return results;
        }, { variant, width, speeds: { walk: walkingSpeed({ width, height }), run: RUN_SPEED },
        ...
        for (const r of result) {
          ...
          assert(Math.abs(r.distance - (r.pace === "walk" ? walkingSpeed({ width, height }) : RUN_SPEED) * 1.2) < 0.001, `No movement regression: ${JSON.stringify(r)}`);
