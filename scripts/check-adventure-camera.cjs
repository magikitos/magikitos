  ...
  for (const scene of Object.values(world.scenes))
    for (const ratio of [NaN, 0.01, 0.45, 0.7, 1, 2]) {
      ...
      if (scene.indoor) {
        ...
        if(ratio === 0.01) assert(Math.abs(metrics.width-scene.width*TILE)<1e-7 || Math.abs(metrics.height-scene.height*TILE)<1e-7,"Zoom out reaches the actual map boundary, not an artificial minimum");
