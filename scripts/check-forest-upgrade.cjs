...
reaction(next);
assert.equal(state.inventory.mushroom, 5, "Three plus two visible mushrooms stack in one counter");
...
assert.equal(reaction(node, { now: 101 * cycle }), null, "Never collect a partial trio");
state.inventory.mushroom = 96;
