...
function verify(directory, id) {
  ...
  for (const rel of expected) {
    if (
      !/^(game-contract\.json|pages\/(es|en|de|fr|it|pt)\.html|assets\/(audio\/(catalog\.json|[a-f0-9]{32}\.mp3)|js\/[a-z.-]+\.js|css\/[a-z.-]+\.css|fonts\/[\w.-]+\.(woff2|txt)|aventura\/(manifest\.json|packs\/[a-z0-9-]+\.(json|png))))$/.test(
        rel,
      )
    )
